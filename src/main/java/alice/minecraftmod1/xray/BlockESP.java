...
public class BlockESP
{
	...
	private KeyBinding _toggleBlockESPBind = new KeyBinding("Toggle BlockESP", Keyboard.KEY_0, Reference.NAME); 
	private KeyBinding _toggleBlockESPGUIBind = new KeyBinding("Toggle BlockESP GUI", Keyboard.KEY_9, Reference.NAME);
	...
	public void init(FMLInitializationEvent event) 
	{
		...
		ESPBlock.setStandardList();
		ESPBlock.removeInvalidBlocks();
	}
	...
	public void keyboardEvent(InputEvent.KeyInputEvent key) 
	{
		...
		// xray bind pressed 
		if(this._toggleBlockESPBind.isPressed()) 
		{
		...
		if(this._toggleBlockESPGUIBind.isPressed()) 
		{
        ...
        for (int i = x - this.Radius; i <= x + this.Radius; ++i) 
        {
            ...
            for (int j = z - this.Radius; j <= z + this.Radius; ++j) 
            {
                ...
                //for (int k = (int) player.posY - this.Radius; k <= (int) player.posY + this.Radius; ++k) 
                {
                    ...
                    for (ESPBlock block : ESPBlock.Blocks) 
                    {
                        if (!block.enabled)
                        	continue;
	...
	private void renderBlock(int x, int y, int z, ESPBlock block) {
